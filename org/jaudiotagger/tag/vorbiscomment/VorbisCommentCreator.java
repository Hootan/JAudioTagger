...
import static org.extra.StandardCharsets.UTF_8;
...
public class VorbisCommentCreator extends AbstractTagCreator
{
    ...
    public ByteBuffer convert(Tag tag, int padding) throws UnsupportedEncodingException
    {
        try
        {
            ...
            int vendorLength = Utils.toBytes(vendorString, UTF_8).length;
            baos.write(org.jaudiotagger.audio.generic.Utils.getSizeLEInt32(vendorLength));
            baos.write(Utils.toBytes(vendorString, UTF_8));

            //User Comment List
            int listLength = tag.getFieldCount() - 1; //Remove Vendor from count         
            baos.write(org.jaudiotagger.audio.generic.Utils.getSizeLEInt32(listLength));
