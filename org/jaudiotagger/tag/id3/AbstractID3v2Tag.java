...
public abstract class AbstractID3v2Tag extends AbstractID3Tag implements Tag
{
     ...
     * @throws IOException
     */
    private static boolean isID3V2Header(RandomAccessFileProvider raf) throws IOException
    {
    ...
    private static boolean isID3V2Header(FileChannelProvider fc) throws IOException
    {
        long start = fc.position();
        ByteBuffer headerBuffer = org.jaudiotagger.audio.generic.Utils.readFileDataIntoBufferBE(fc, FIELD_TAGID_LENGTH);
        fc.position(start);
        String s = org.jaudiotagger.audio.generic.Utils.readThreeBytesAsChars(headerBuffer);
     ...
     * @throws IOException
     */
    public static boolean isId3Tag(RandomAccessFileProvider raf) throws IOException
    {
     ...
     * @throws IOException
     */
    public static boolean isId3Tag(FileChannelProvider fc) throws IOException
    {
        if (!isID3V2Header(fc))
        {
            return false;
    ...
    */
    ...
    public void delete(RandomAccessFileProvider file) throws IOException
    {
        // this works by just erasing the "ID3" tag at the beginning
        // of the file
        byte[] buffer = new byte[FIELD_TAGID_LENGTH];
        //Read into Byte Buffer
        final FileChannelProvider fc = file.getChannelProvider();
        fc.position();
     ...
     * @throws IOException
     */
    public abstract long write(FileSystemProvider provider, long audioStartByte) throws IOException;
     ...
     */
    protected FileLock getFileLockForWriting(FileChannelProvider fileChannel, String filePath) throws IOException
    {
     ...
     */
    public void write(RandomAccessFileProvider file) throws IOException
    {
    }
     ...
     * @throws IOException
     */
    public static long getV2TagSizeIfExists(FileSystemProvider provider) throws IOException
    {
     ...
     * @param audioStart
     * @param provider        The file to adjust the padding length of
     * @throws FileNotFoundException if the file exists but is a directory
     *                               rather than a regular file or cannot be opened for any other
     *                               reason
     * @throws IOException           on any I/O error
     */
    public void adjustPadding(FileSystemProvider provider, int paddingSize, long audioStart) throws FileNotFoundException, IOException
    {
        logger.finer("Need to move audio file to accommodate tag");
        FileChannelProvider fcIn = null;
        FileChannelProvider fcOut;
        ...
        FileSystemProvider paddedProvider;

        try
        {
            paddedProvider = provider.createTempFile(org.jaudiotagger.audio.generic.Utils.getBaseFilenameForTempFile(provider), ".new", provider.getParentFile());
            logger.finest("Created temp file:" + paddedProvider.getName() + " for " + provider.getName());
        }
        ...
        catch (IOException ioe)
        {
            ...
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_CREATE_TEMPORARY_FILE_IN_FOLDER.getMsg(provider.getName(), provider.getParentFile().getPath()));
                throw new UnableToCreateFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_CREATE_TEMPORARY_FILE_IN_FOLDER.getMsg(provider.getName(), provider.getParentFile().getPath()));
            }
            else
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_CREATE_TEMPORARY_FILE_IN_FOLDER.getMsg(provider.getName(), provider.getParentFile().getPath()));
        ...
        try
        {
            fcOut = paddedProvider.getRandomAccessFile("rw").getChannelProvider();
        }
        //Vista:Can occur if have special permission Create Folder/Append Data denied
        catch (FileNotFoundException ioe)
        {
            logger.log(Level.SEVERE, ioe.getMessage(), ioe);
            logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_MODIFY_TEMPORARY_FILE_IN_FOLDER.getMsg(provider.getName(), provider.getParentFile().getPath()));
            throw new UnableToModifyFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_MODIFY_TEMPORARY_FILE_IN_FOLDER.getMsg(provider.getName(), provider.getParentFile().getPath()));
        }
        ...
        try
        {
            ...
            fcIn = provider.getRandomAccessFile("r").getChannelProvider();
            ...
            logger.finer("Copying:" + (provider.length() - audioStart) + "bytes");

            //If the amount to be copied is very large we split into 10MB lumps to try and avoid
            //out of memory errors
            long audiolength = provider.length() - audioStart;
            ...
            long lastModified = provider.lastModified();
            ...
            replaceFile(provider, paddedProvider);

            //Update modification time
            //TODO is this the right file ?
            paddedProvider.setLastModified(lastModified);
        }
        catch (UnableToRenameFileException ure)
        {
            paddedProvider.delete();
            throw ure;
        }
        finally
        {
     ...
     * @throws IOException
     */
    protected void writeBufferToFile(FileSystemProvider provider, ByteBuffer headerBuffer, byte[] bodyByteBuffer, int padding, int sizeIncPadding, long audioStartLocation) throws IOException
    {
        FileChannelProvider fc = null;
        ...
        if (sizeIncPadding > audioStartLocation)
        {
            ...
            adjustPadding(provider, sizeIncPadding, audioStartLocation);
        }

        try
        {
            fc = provider.getRandomAccessFile("rw").getChannelProvider();
            fileLock = getFileLockForWriting(fc, provider.getPath());
        ...
        catch (FileNotFoundException fe)
        {
            ...
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getPath()));
                throw new UnableToModifyFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getPath()));
            }
            else
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getPath()));
                throw new UnableToCreateFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getPath()));
            }
        }
        catch (IOException ioe)
        {
            ...
            if (ioe.getMessage().equals(FileSystemMessage.ACCESS_IS_DENIED.getMsg()))
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getParentFile().getPath()));
                throw new UnableToModifyFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getParentFile().getPath()));
            }
            else
            {
                logger.severe(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getParentFile().getPath()));
                throw new UnableToCreateFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_OPEN_FILE_FOR_EDITING.getMsg(provider.getParentFile().getPath()));
            }
        }
        finally
        {
     ...
     * @throws IOException
     */
    private void replaceFile(FileSystemProvider originalProvider, FileSystemProvider newProvider) throws IOException
    {
        ...
        renameOriginalResult = originalProvider.renameTo(originalProviderBackup);
        if (!renameOriginalResult)
        {
            logger.warning(ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_ORIGINAL_FILE_TO_BACKUP.getMsg(originalProvider.getAbsolutePath(), originalProviderBackup.getName()));
            newProvider.delete();
            throw new UnableToRenameFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_ORIGINAL_FILE_TO_BACKUP.getMsg(originalProvider.getAbsolutePath(), originalProviderBackup.getName()));
        }

        //Rename new Temporary file to the final file
        boolean renameResult = newProvider.renameTo(originalProvider);
        if (!renameResult)
        {
            ...
            renameOriginalResult = originalProviderBackup.renameTo(originalProvider);
            if (!renameOriginalResult)
            {
            ...
            newProvider.delete();
            throw new UnableToRenameFileException(ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_TO_ORIGINAL_FILE.getMsg(originalProvider.getAbsolutePath(), newProvider.getName()));
        }
        else
        {
    ...
    protected TagField doCreateTagField(FrameAndSubId formatKey, String... values) throws KeyNotFoundException, FieldDataInvalidException
    {
        ...
        if (frame.getBody() instanceof FrameBodyUFID)
        {
            ...
            try
            {
                ((FrameBodyUFID) frame.getBody()).setUniqueIdentifier(Utils.toBytes(value, StandardCharsets.ISO_8859_1));
            }
            ...
            while (li.hasNext())
            {
                ...
                else if (next instanceof FrameBodyUFID)
                {
                    if (((FrameBodyUFID) next).getOwner().equals(formatKey.getSubId()))
                    {
                        ...
                        {
                            values.add(Utils.getString(((FrameBodyUFID) next).getUniqueIdentifier()));
                        }
                    }
                }
    ...
    public TagField createLinkedArtworkField(String url)
    {
        ...
        if (frame.getBody() instanceof FrameBodyAPIC)
        {
            ...
            body.setObjectValue(DataTypes.OBJ_PICTURE_DATA, Utils.getBytes(url, StandardCharsets.ISO_8859_1));
