...
public class TextEncoding extends AbstractIntStringValuePair
{
    ...
    private final SparseArray<Charset> idToCharset = new SparseArray<>();
    ...
    private TextEncoding()
    {
        ...
        for (int i=0; i < idToCharset.size(); i++) {
            int key = idToCharset.keyAt(i);
            Charset val = idToCharset.get(key);
            idToValue.put(key, val.name());
        }
