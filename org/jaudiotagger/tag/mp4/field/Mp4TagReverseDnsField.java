...
public class Mp4TagReverseDnsField extends Mp4TagField implements TagTextField
{
    ...
    {
        return Utils.toBytes(content, getEncoding());
    }
    ...
    public byte[] getRawContent() throws UnsupportedEncodingException
    {
        try
        {
            ...
            //Create Meanbox data
            byte[] issuerRawData = Utils.toBytes(issuer, getEncoding());
            baos.write(org.jaudiotagger.audio.generic.Utils.getSizeBEInt32(Mp4BoxHeader.HEADER_LENGTH + Mp4MeanBox.PRE_DATA_LENGTH + issuerRawData.length));
            baos.write(Utils.toBytes(Mp4MeanBox.IDENTIFIER, StandardCharsets.ISO_8859_1));
            baos.write(new byte[]{0, 0, 0, 0});
            baos.write(issuerRawData);

            //Create Namebox data
            byte[] nameRawData = Utils.toBytes(descriptor, getEncoding());
            baos.write(org.jaudiotagger.audio.generic.Utils.getSizeBEInt32(Mp4BoxHeader.HEADER_LENGTH + Mp4NameBox.PRE_DATA_LENGTH + nameRawData.length));
            baos.write(Utils.toBytes(Mp4NameBox.IDENTIFIER, StandardCharsets.ISO_8859_1));
            baos.write(new byte[]{0, 0, 0, 0});
            ...
            outerbaos.write(org.jaudiotagger.audio.generic.Utils.getSizeBEInt32(Mp4BoxHeader.HEADER_LENGTH + baos.size()));
            outerbaos.write(Utils.toBytes(IDENTIFIER, StandardCharsets.ISO_8859_1));
    ...
    public byte[] getRawContentDataOnly() throws UnsupportedEncodingException
    {
        ...
        try
        {
            //Create DataBox data
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            byte[] dataRawData = Utils.toBytes(content, getEncoding());
            baos.write(org.jaudiotagger.audio.generic.Utils.getSizeBEInt32(Mp4BoxHeader.HEADER_LENGTH + Mp4DataBox.PRE_DATA_LENGTH + dataRawData.length));
            baos.write(Utils.toBytes(Mp4DataBox.IDENTIFIER, StandardCharsets.ISO_8859_1));
