...
public class ChunkHeader
{
     ...
     */
    public boolean readHeader(final FileChannelProvider fc) throws IOException
    {
        ...
        header.position(0);
        this.chunkId  = org.jaudiotagger.audio.generic.Utils.readFourBytesAsChars(header);
     ...
     * @return {@code true}, if we were able to read a chunk header and believe we found a valid chunk id.
     */
    public boolean readHeader(final RandomAccessFileProvider raf) throws IOException
    {
        ByteBuffer header = ByteBuffer.allocate(CHUNK_HEADER_SIZE);
        startLocationInFile = raf.getFilePointer();
        raf.getChannelProvider().read(header);
        header.order(byteOrder);
        header.position(0);
        this.chunkId  = org.jaudiotagger.audio.generic.Utils.readFourBytesAsChars(header);
        this.size = header.getInt();
    ...
    public ByteBuffer writeHeader()
    {
        ...
        bb.order(byteOrder);
        bb.put(Utils.getBytes(chunkId, StandardCharsets.US_ASCII));
        bb.putInt((int)size);
        bb.flip();
        return bb;
    }
