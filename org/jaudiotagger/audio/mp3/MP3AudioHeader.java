...
public class MP3AudioHeader implements AudioHeader
{
     ...
     */
    public MP3AudioHeader(final FileSystemProvider seekProvider) throws IOException, InvalidAudioFrameException
    {
        if (!seek(seekProvider, 0))
        {
            throw new InvalidAudioFrameException("No audio header found within" + seekProvider.getName());
        }
    }
     ...
     * @throws InvalidAudioFrameException
     */
    public MP3AudioHeader(final FileSystemProvider seekProvider, long startByte) throws IOException, InvalidAudioFrameException
    {
        if (!seek(seekProvider, startByte))
        {
            throw new InvalidAudioFrameException(ErrorMessage.NO_AUDIO_HEADER_FOUND.getMsg(seekProvider.getName()));
        }
    }
     ...
     *
     * @param seekProvider  MP3 file to seek
     ...
     */
    public boolean seek(final FileSystemProvider seekProvider, long startByte) throws IOException
    {
        ...
        final RandomAccessFileProvider fis = seekProvider.getRandomAccessFile("r");
        final FileChannelProvider fc = fis.getChannelProvider();
            ...
            do
            {
                ...
                if (MPEGFrameHeader.isMPEGFrame(bb))
                {
                    try
                    {
                        ...
                        {
                            syncFound = isNextFrameValid(seekProvider, filePointerCount, bb, fc);
                            if (syncFound)
                            {
                                break;
                            }
        ...
        setFileSize(seekProvider.length());
     ...
     * @throws IOException
     */
    private boolean isNextFrameValid(FileSystemProvider seekProvider, long filePointerCount, ByteBuffer bb, FileChannelProvider fc) throws IOException
    {
        if (MP3AudioHeader.logger.isLoggable(Level.FINEST))
        {
            MP3AudioHeader.logger.finer("Checking next frame" + seekProvider.getName() + ":fpc:" + filePointerCount + "skipping to:" + (filePointerCount + mp3FrameHeader.getFrameLength()));
        }
    ...
    protected void setBitRate()
    {
        ...
        if (mp3XingFrame != null && mp3XingFrame.isVbr())
        {
            ...
            {
                bitrate = ((mp3XingFrame.getAudioSize() * CONVERTS_BYTE_TO_BITS) / (timePerFrame * getNumberOfFrames() * CONVERT_TO_KILOBITS));
            }
            else
            {
                bitrate = (((fileSize - startByte) * CONVERTS_BYTE_TO_BITS) / (timePerFrame * getNumberOfFrames() * CONVERT_TO_KILOBITS));
            }
        }
        else if (mp3VbriFrame != null)
        {
    ...
    public String getBitRate()
    {
        if (mp3XingFrame != null && mp3XingFrame.isVbr())
        {
            return isVbrIdentifier + getBitrateString();
        }
        else if (mp3VbriFrame != null)
        {
            return isVbrIdentifier + getBitrateString();
    ...
    private String getBitrateString() {
        return String.valueOf(Math.round(Double.parseDouble(Double.toString(bitrate))));
    }
    ...
    public int getChannelMode() {
        return mp3FrameHeader.getChannelMode();
    }
    ...
    @Override
    public String getVideoCodecIdAsString() {
        return null;
    }
