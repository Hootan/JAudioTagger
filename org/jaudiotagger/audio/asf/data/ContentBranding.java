...
public final class ContentBranding extends MetadataContainer
{
    ...
    {
        if (org.jaudiotagger.audio.asf.util.Utils.isBlank(imageURL))
        {
    ...
    {
        if (org.jaudiotagger.audio.asf.util.Utils.isBlank(copyRight))
        {
    ...
    public long writeInto(final OutputStream out) throws IOException
    {
        ...
        org.jaudiotagger.audio.asf.util.Utils.writeUINT64(chunkSize, out);
        org.jaudiotagger.audio.asf.util.Utils.writeUINT32(getImageType(), out);
        assert getImageType() >= 0 && getImageType() <= 3;
        final byte[] imageData = getImageData();
        assert getImageType() > 0 || imageData.length == 0;
        org.jaudiotagger.audio.asf.util.Utils.writeUINT32(imageData.length, out);
        out.write(imageData);
        org.jaudiotagger.audio.asf.util.Utils.writeUINT32(getBannerImageURL().length(), out);
        out.write(Utils.toBytes(getBannerImageURL(), StandardCharsets.US_ASCII));
        org.jaudiotagger.audio.asf.util.Utils.writeUINT32(getCopyRightURL().length(), out);
        out.write(Utils.toBytes(getCopyRightURL(), StandardCharsets.US_ASCII));
        return chunkSize;
    }
