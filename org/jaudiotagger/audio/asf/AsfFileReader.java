...
public class AsfFileReader extends AudioFileReader
{
    ...
    private GenericAudioHeader getAudioHeader(final AsfHeader header) throws CannotReadException
    {
        ...
        //What if it's video?
        AudioStreamChunk audioChunk = header.getAudioStreamChunk();
        if (audioChunk != null) {
            info.setBitRate(audioChunk.getKbps());
            info.setChannelNumber((int) audioChunk.getChannelCount());
            info.setEncodingType("ASF (audio): " + audioChunk.getCodecDescription());
            info.setLossless(audioChunk.getCompressionFormat() == AudioStreamChunk.WMA_LOSSLESS);
            info.setPreciseLength(header.getFileHeader().getPreciseDuration());
            info.setSamplingRate((int) audioChunk.getSamplingRate());
        ...
        for (Chunk chunk : streamChunks) {
            if (chunk instanceof VideoStreamChunk) {
                info.setCodecIdAsString(((VideoStreamChunk) chunk).getCodecIdAsString());
                info.setPictureWidth((int) ((VideoStreamChunk) chunk).getPictureWidth());
                info.setPictureHeight((int) ((VideoStreamChunk) chunk).getPictureHeight());

                /*int duration = header.get.getTotalDuration();
                int frameRate = (int) Math.ceil(vMeta.getTotalFrames() / duration);
     ...
     * (overridden)
     *
     * @see AudioFileReader#getEncodingInfo(RandomAccessFileProvider)
     */
    @Override
    protected GenericAudioHeader getEncodingInfo(final RandomAccessFileProvider raf) throws CannotReadException, IOException
    {
        raf.seek(0);
     ...
     * (overridden)
     *
     * @see AudioFileReader#getTag(RandomAccessFileProvider)
     */
    @Override
    protected AsfTag getTag(final RandomAccessFileProvider raf) throws CannotReadException, IOException
    {
        raf.seek(0);
        AsfTag tag;
        try
     ...
     */
    @Override
    public AudioFile read(final FileSystemProvider f) throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        if (!f.canRead())
        {
        ...
        try
        {
            stream = new FullRequestInputStream(new BufferedInputStream(f.getInputStream(0)));
