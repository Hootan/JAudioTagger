...
public class RealFileReader extends AudioFileReader
{
    ...
    protected GenericAudioHeader getEncodingInfo(RandomAccessFileProvider raf) throws CannotReadException, IOException
    {
        ...
        if (objVersion == 0)
        {
            final double maxBitRate = Utils.readUint32(dis) / Utils.KILOBYTE_MULTIPLIER;
            final double avgBitRate = Utils.readUint32(dis) / Utils.KILOBYTE_MULTIPLIER;
    ...
    private RealChunk findPropChunk(RandomAccessFileProvider raf) throws IOException, CannotReadException
    {
    ...
    private RealChunk findContChunk(RandomAccessFileProvider raf) throws IOException, CannotReadException
    {
        final RealChunk rmf = RealChunk.readChunk(raf);
        final RealChunk prop = RealChunk.readChunk(raf);
