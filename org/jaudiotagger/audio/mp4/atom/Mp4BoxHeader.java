...
public class Mp4BoxHeader
{
    ...
    public Mp4BoxHeader(String id)
    {
        ...
        {
            this.id    = id;
            dataBuffer.put(4, Utils.toBytes(id, StandardCharsets.ISO_8859_1)[0]);
            dataBuffer.put(5, Utils.toBytes(id, StandardCharsets.ISO_8859_1)[1]);
            dataBuffer.put(6, Utils.toBytes(id, StandardCharsets.ISO_8859_1)[2]);
            dataBuffer.put(7, Utils.toBytes(id, StandardCharsets.ISO_8859_1)[3]);
        }
    ...
    public void update(ByteBuffer headerData)
    {
        ...
        this.id = org.jaudiotagger.audio.generic.Utils.readFourBytesAsChars(dataBuffer);
    ...
    {
        byte[] headerSize = org.jaudiotagger.audio.generic.Utils.getSizeBEInt32(length);
     ...
     * @return
     */
    public static Mp4BoxHeader seekWithinLevel(RandomAccessFileProvider fc, String id) throws IOException
    {
        logger.finer("Started searching for:" + id + " in file at:" + fc.getFilePointer());

        Mp4BoxHeader boxHeader = new Mp4BoxHeader();
        ByteBuffer headerBuffer = ByteBuffer.allocate(HEADER_LENGTH);
        int bytesRead = fc.read(headerBuffer.array());
        ...
        {
            logger.finer("Found:" + boxHeader.getId() + " Still searching for:" + id + " in file at:" + fc.getFilePointer());
            ...
            fc.seek(fc.getFilePointer() + boxHeader.getDataLength());
            if (fc.getFilePointer() > fc.length())
            {
                return null;
            }
            headerBuffer.rewind();
            bytesRead = fc.read(headerBuffer.array());
