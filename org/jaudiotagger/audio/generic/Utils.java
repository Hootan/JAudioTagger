...
import static org.extra.StandardCharsets.ISO_8859_1;
import static org.extra.StandardCharsets.US_ASCII;
...
public class Utils
{
    ...
    public static double KILOBYTE_MULTIPLIER = 1000d;
     ...
     */
    /*public static String getExtension(final FileSystemProvider f)
    {
     ...
     * @return The extension of the given file
     */
	public static String getMagicExtension(final FileSystemProvider f) throws IOException{
    ...
    public static String readPascalString(final ByteBuffer bb) throws IOException {
        ...
        bb.get(buf);
        return org.extra.Utils.toString(buf, 0, len, ISO_8859_1);
    }
    ...
    public static String getString(final ByteBuffer buffer, final int offset, final int length, final Charset encoding)
    {
        ...
        buffer.get(b);
        return org.extra.Utils.getString(b, 0, length, encoding);
    }
    ...
    public static String getString(final ByteBuffer buffer, final Charset encoding)
    {
        ...
        buffer.get(b);
        return org.extra.Utils.getString(b, 0, b.length, encoding);
    }

    /**
     * Read a 32-bit big-endian unsigned integer using a RandomAccess.
     *
     * Reads 4 bytes but returns as long
     */
    public static long readUint32(final RandomAccessFileProvider di) throws IOException
    {
        final byte[] buf8 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        di.readFully(buf8, 4, 4);
        return ByteBuffer.wrap(buf8).getLong();
    }
    public static long readUint32(final DataInputStream di) throws IOException
    {
        final byte[] buf8 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        di.readFully(buf8, 4, 4);
        return ByteBuffer.wrap(buf8).getLong();
     ...
     */
    public static int readUint16(final DataInputStream di) throws IOException
    {
     ...
     */
    public static String readString(final RandomAccessFileProvider di, final int charsToRead) throws IOException
    {
        final byte[] buf = new byte[charsToRead];
        di.readFully(buf);
        return org.extra.Utils.toString(buf, US_ASCII);
    }
    public static String readString(final DataInputStream di, final int charsToRead) throws IOException
    {
        final byte[] buf = new byte[charsToRead];
        di.readFully(buf);
        return org.extra.Utils.toString(buf, US_ASCII);
     ...
     * @return
     */
    public static String getBaseFilenameForTempFile(final FileSystemProvider provider)
    {
        final String filename = getMinBaseFilenameAllowedForTempFile(provider);
     ...
     *         creation.
     */
    public static String getMinBaseFilenameAllowedForTempFile(final FileSystemProvider provider)
    {
        final String s = AudioFile.getBaseFilename(provider);
        if (s.length() >= 3)
        {
            return s;
        }
     ...
     * @return
     */
    public static boolean rename(final FileSystemProvider fromProvider, final FileSystemProvider toProvider)
    {
        logger.log(Level.CONFIG,"Renaming From:"+ fromProvider.getAbsolutePath() + " to "+ toProvider.getAbsolutePath());

        /*Removed  SD card always says true right after delete.
        if(toProvider.exists()) {
            logger.log(Level.SEVERE,"Destination File:"+ toProvider + " already exists");
            return false;
        }*/

        //Rename File, could fail because being  used or because trying to rename over filesystems
        final boolean result = fromProvider.renameTo(toProvider);
        if (!result)
        {
            // Might be trying to rename over filesystem, so try copy and delete instead
            if (copy(fromProvider, toProvider))
            {
                ...
                boolean deleteResult= fromProvider.delete();
                if(!deleteResult) {
                    logger.log(Level.SEVERE,"Unable to delete File:"+ fromProvider);
                    toProvider.delete();
                    return false;
                }
                return true;
     ...
     *
     * @param fromProvider The existing File
     * @param toProvider   The new File
     * @return <code>true</code> if and only if the renaming succeeded;
     *         <code>false</code> otherwise
     */
    public static boolean copy(final FileSystemProvider fromProvider, final FileSystemProvider toProvider)
    {
        try
        {
            copyThrowsOnException(fromProvider, toProvider);
            return true;
        }
        catch (IOException e)
        {
    ...
    public static String readFourBytesAsChars(final ByteBuffer bytes)
    {
        byte[] b = new byte[4];
        bytes.get(b);
        return org.extra.Utils.getString(b, ISO_8859_1);
    }
     ...
     * @throws IOException
     */
    public static ByteBuffer readFileDataIntoBufferLE(FileChannelProvider fc, final int size) throws IOException
    {
     ...
     * @throws IOException
     */
    public static ByteBuffer readFileDataIntoBufferBE(FileChannelProvider fc, final int size) throws IOException
    {
        final ByteBuffer tagBuffer = ByteBuffer.allocateDirect(size);
        fc.read(tagBuffer);
        tagBuffer.position(0);
     ...
     */
    public static void copyThrowsOnException(final FileSystemProvider source, final FileSystemProvider destination) throws IOException {
        ...
        RandomAccessFileProvider inStream = source.getRandomAccessFile("r");
        try {
            RandomAccessFileProvider outStream = destination.getRandomAccessFile("rw");
            try {
                final FileChannelProvider inChannel = inStream.getChannelProvider();
                final FileChannelProvider outChannel = outStream.getChannelProvider();
