...
public abstract class AudioFileReader
{
    ...
    * The RandomAccess must be kept open after this function, but can point
    * at any offset in the file.
    *
    * @param raf The RandomAccess associtaed with the current file
    * @exception IOException is thrown when the RandomAccess operations throw it (you should never throw them manually)
    * @exception CannotReadException when an error occured during the parsing of the encoding infos
    */
    protected abstract GenericAudioHeader getEncodingInfo(RandomAccessFileProvider raf) throws CannotReadException, IOException;
    ...
    /*
      ...
      *
      * @param raf The RandomAccess associted with the current file
      * @exception IOException is thrown when the RandomAccess operations throw it (you should never throw them manually)
      * @exception CannotReadException when an error occured during the parsing of the tag
      */
    protected abstract Tag getTag(RandomAccessFileProvider raf) throws CannotReadException, IOException;
    ...
    public AudioFile read(FileSystemProvider provider) throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        ...
        {
            logger.config(ErrorMessage.GENERAL_READ.getMsg(provider.getAbsolutePath()));
        }
        ...
        {
            throw new CannotReadException(ErrorMessage.GENERAL_READ_FAILED_FILE_TOO_SMALL.getMsg(provider.getAbsolutePath()));
        }

        RandomAccessFileProvider raf = null;
        try
        {
            raf = provider.getRandomAccessFile("r");
            raf.seek(0);
        ...
        {
            logger.log(Level.SEVERE, ErrorMessage.GENERAL_READ.getMsg(provider.getAbsolutePath()),e);
            throw new CannotReadException(provider.getAbsolutePath()+":" + e.getMessage(), e);
        }
        finally
        {
            try
            ...
            {
                logger.log(Level.WARNING, ErrorMessage.GENERAL_READ_FAILED_UNABLE_TO_CLOSE_RANDOM_ACCESS_FILE.getMsg(provider.getAbsolutePath()));
            }
        }
    }
}
