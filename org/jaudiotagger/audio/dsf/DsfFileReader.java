...
public class DsfFileReader extends AudioFileReader2
{
        ...
        try {
            ...
            if (dsd != null) {
                ...
                if (fmt != null) {
                    ...
                    throw new CannotReadException(provider + " Not a valid dsf file. Content does not include 'fmt ' chunk");
                }
            } else {
                throw new CannotReadException(provider + " Not a valid dsf file. Content does not start with 'DSD '");
            }
        } finally {
            fc.close();
        }
    ...
    protected Tag getTag(FileSystemProvider provider) throws CannotReadException, IOException
    {
        ...
        try {
            DsdChunk dsd = DsdChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH));
            if (dsd != null) {
                return readTag(fc, dsd, provider.toString());
            } else {
                throw new CannotReadException(provider + " Not a valid dsf file. Content does not start with 'DSD '.");
            }
        } finally {
     ...
     */
    private Tag readTag(FileChannelProvider fc, DsdChunk dsd, String fileName) throws CannotReadException,IOException
    {
